...
class Network:
    ...
    def computeSpikesRate(self, bin_width, spikes_i, plot):
        #spikes_i_np = np.array(spikes_i, dtype=object)
        #plot(spikes_i_np)
        #spikes_i_combined = spikes_i_np.flatten()

        spikes_i_combined = []
        for neuron in spikes_i:
            for spk in neuron:
                spikes_i_combined.append(spk-self.i_interval*self.t_inter) # map the timing of spike in the [0;5000]s range

        spikes_i_combined.sort()

        time_bins = np.arange(0, self.t_inter + bin_width, bin_width)
        spike_counts = np.zeros(len(time_bins) - 1)

        # Calculate spikes rates
        for spk in spikes_i_combined:
            bin_index = np.searchsorted(time_bins, spk) - 1
            if 0 <= bin_index < len(spike_counts):
                spike_counts[bin_index] += 1
        
        spk_rate = spike_counts*1000/(bin_width*len(spikes_i))

        # Plot Firing Rates        
        if plot==True:
            plt.bar(range(0, self.t_inter, bin_width), spk_rate, 2)
            plt.title('Firing rates of the cortex over time bins of 10 ms') 
            plt.xlabel('Time (ms)')
            plt.ylabel('Spike Rate (spikes/s)')
            plt.ylim([0, 70])
            plt.show()
        
        print("Mean FR cortex : ", np.mean(np.array(spk_rate)))
        return spk_rate
    ...
    def computePSD(self, spikes_t, plot):
        ...
        if plot==True:
            fig, ax = plt.subplots()
            ax.plot(f, PSD)        # Plot the planning period,
    ...
    def interFunc(self, t):
        ...
        # MFR calculations
        #self.computeSpikesRate(10, cortex_spikes_i, plot=False)
